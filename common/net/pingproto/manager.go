...
func (m *PingManager) GetClient(destination net.Destination) *pingClient {
	...
	client := &pingClient{
		...
		timer: signal.CancelAfterInactivity(ctx, func() {
			cancel()
			if client, loaded := m.clientTable.LoadAndDelete(key); loaded {
				common.Close(client)
			}
		}, 5*time.Minute),
		dest: destination,
...
func (m *PingManager) loop4() {
	...
	for {
		...
		if !loaded {
			newError("callback missing for icmp4 packet from ", buffer.Endpoint.Address).WriteToLog()
			continue
		}
...
func (m *PingManager) loop6() {
	...
	for {
		...
		callbackI, loaded := client.callbacks.LoadAndDelete(hdr.Sequence())
		if !loaded {
			newError("callback missing for icmp6 packet from ", buffer.Endpoint.Address).WriteToLog()
			continue
		}
		callback := callbackI.(*pingCallback)
		callback.data = hdr
...
func (c *pingClient) Close() error {
	c.callbacks.Range(func(key, value interface{}) bool {
		common.Close(value)
		return true
	})
	return nil
...
func (c *pingConn4) Read(b []byte) (n int, err error) {
	...
	newError("read ping request from ", c.dest.Address, " seq ", callback.sequence, " id ", hdr.Ident()).AtDebug().WriteToLog()
	if hdr.Ident() != callback.id {
...
func (c *pingConn4) Write(b []byte) (n int, err error) {
	...
	newError("write ping request to ", c.dest.Address, " seq ", sequence, " id ", callback.id).AtDebug().WriteToLog()
...
func (c *pingConn6) Read(b []byte) (n int, err error) {
	...
	newError("read ping request from ", c.dest.Address, " seq ", callback.sequence, " id ", callback.id).AtDebug().WriteToLog()
	if hdr.Ident() != callback.id {
